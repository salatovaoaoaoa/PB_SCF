...
def create_in(
    ...
    for str in range(len(data)):
        ...
        if 'state : H3O : alphabulk' in data[str]:
            data[str] = f'state : H3O : alphabulk : {(1 + 10**(pH_b_scf))**(-1)}\n'
    ...
    full_file_name = f'{file_name_prefix}_Cs_{round(Cs,5)}_R_sp_{r_sphere}_N_{N}_theta_{round(theta,2)}_chi_{chi}_alpha_{alpha}_pH_{pH_b_scf}.in'
