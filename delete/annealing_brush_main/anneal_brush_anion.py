...
def annealing_anion(
    ...
    lb: float = 1,
    a: float = 1,
    delta_pK: float = 0.3,  # Отступ от pK щетки
    pK: float = 5,
    # Параметры белка
    ...
    z_in_range = np.linspace(0, H, num=round(H-minus_layers))
    z_out_range = np.linspace(H, H + 20, num=20)
    ...
    def psi_in_z(z):
        return -1 * alpha_b * (H**2 - z**2) / (H_0**2) + np.log(
            (1 - alpha_b)
            / (alpha_b * (1 - alpha_H))
            * (1 - (1 - alpha_H) * np.exp(alpha_b * (H**2 - z**2) / (H_0**2)))
        )
    ...
    def psi_in_0():
        return -1 * alpha_b * (H**2) / (H_0**2) + np.log(
            (1 - alpha_b)
            / (alpha_b * (1 - alpha_H))
            * (1 - (1 - alpha_H) * np.exp(alpha_b * (H**2) / (H_0**2)))
        )
    ...
    def alpha_minus_out():
        return (
            ...
            + (1 - alpha_buf_minus(pK_minus, pH_b))
            / (alpha_buf_minus(pK_minus, pH_b))
            * np.exp(-1 * psi_out_z(z_out_range))
        ) ** (-1)
    ...
    f_ion_in = delta_F_ion(
        psi_in_z(z_in_range), alpha_buf_plus_fast(), alpha_buf_minus_fast()
    )
    f_ion_out = delta_F_ion(
        psi_out_z(z_out_range), alpha_buf_plus_fast(), alpha_buf_minus_fast()
    )
    ...
    charge_in = Q_exp(
        alpha_buf_plus_fast(), alpha_buf_minus_fast(), psi_in_z(z_in_range)
    )

    charge_out = Q_exp(
        alpha_buf_plus_fast(), alpha_buf_minus_fast(), psi_out_z(z_out_range)
    )
    ...
    def rho(z):
        ...
        numerator = 1 - (1 - alpha_H) * (1 + 2 * alpha_b * z**2 / H_0**2) * exp(
            alpha_b * (H**2 - z**2) / H_0**2
        )
    ...
    polymer_dens_anneal = np.asarray(
        [
            (-1 * rho_range[i] - Cs * np.exp(psi_in[i]) + Cs * np.exp(-psi_in[i]))
            / (alpha_z[i])
    ...
    F_osm = f_osmotic(polymer_dens_anneal)

    # ВКЛАД В СВОБОДНУЮ ЭНЕРГИЮ ОТ БЕЛКА (ТЕОРИЯ)

    F_vol_in = f_volume_protein(psi_in)
    F_vol_out = f_volume_protein(psi_out)
    ...
    F_osm_SCF = f_osmotic(parse_SCF_phi)

    # ВКЛАД В СВОБОДНУЮ ЭНЕРГИЮ ОТ БЕЛКА (SCF)

    F_vol_SCF = f_volume_protein(parse_SCF_psi)
